...
public class ShopManager : MonoBehaviour
{
    ...
    public void GenerateShopMenu()
    {
        ...
        foreach (DatabaseObject _object in ObjectsDatabase.Instance.GetObjectsDatabase().objectsData)
        {
            ...
            button.GetComponent<Image>().sprite = _object.icon;
            newButton.transform.GetChild(1).GetComponent<TextMeshProUGUI>().text = _object.Name;
            newButton.transform.GetChild(2).GetComponent<TextMeshProUGUI>().text = _object.Cost.ToString();
        }
    }
